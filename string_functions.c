 ...
 * @arg: variable argument list containing the string to be reversed and printed
 * 
...
int rot13(va_list list)
{
    ...
    for (index = 0; original_string[index] != '\0'; index++)
    {
        if (_isalpha(original_string[index])) {
            if ((original_string[index] >= 'a' && original_string[index] <= 'm') || (original_string[index] >= 'A' && original_string[index] <= 'M'))
